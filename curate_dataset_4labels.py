#!/usr/bin/env python3
"""Build a four-label, formatted, token-limited, class-balanced MADE-WIC dataset.
...
Steps:
    ...
    3. Keep OSPR (the source column named W), DEVIGN, BIGVUL, and MAT as
       separate boolean labels.
    4. Count tiktoken tokens and discard functions over the configured limit.
    5. Downsample joint (OSPR, DEVIGN, BIGVUL, MAT) strata to restore the
       pre-filter distribution, then emit Projectname, Filepath, Function, and
       an [OSPR, DEVIGN, BIGVUL, MAT] label.
...
SOURCE_LABEL_COLUMNS = ("W", "Devign", "Big-Vul")
LABEL_COLUMNS = ("OSPR", "DEVIGN", "BIGVUL", "MAT")
ANNOTATION_COLUMNS = ("PS", "MAT", "SecI")
STEP_FILENAMES = {
    1: "01_loaded.csv",
    2: "02_clang_formatted.csv",
    3: "03_labels_separated.csv",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=SCRIPT_DIR / "MADE-WIC" / "Dataset",
        help="Directory containing OSPR, big-vul, and devign folders.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=SCRIPT_DIR / "curated_dataset_4labels_steps",
...
def row_stratum(row: Mapping[str, str]) -> Stratum:
    return tuple(as_bool(row.get(column, "0")) for column in LABEL_COLUMNS)  # type: ignore[return-value]
...
def run_step_3(
    ...
    handle, reader = open_reader(input_path)
    require_columns(
        input_path, reader.fieldnames, (*SOURCE_LABEL_COLUMNS, *ANNOTATION_COLUMNS)
    )
    fieldnames = [
        column for column in (reader.fieldnames or []) if column not in SOURCE_LABEL_COLUMNS
    ]
    function_index = fieldnames.index("Function")
    for column in reversed(LABEL_COLUMNS[:-1]):
        fieldnames.insert(function_index + 1, column)
    distribution: Counter[Stratum] = Counter()
    try:
        with atomic_csv_writer(output_path, fieldnames) as writer:
            rows = 0
            for row in tqdm(
                reader, total=total, desc="Step 3/6 separate labels", unit="rows"
            ):
                row["OSPR"] = str(as_bool(row.get("W", "")))
                row["DEVIGN"] = str(as_bool(row.get("Devign", "")))
                row["BIGVUL"] = str(as_bool(row.get("Big-Vul", "")))
                row["MAT"] = str(as_bool(row.get("MAT", "")))
...
def deserialize_distribution(value: object) -> Counter[Stratum]:
    ...
    for key, count in value.items():
        parts = tuple(int(part) for part in str(key).split(","))
        if len(parts) != len(LABEL_COLUMNS):
            raise ValueError(
                f"Expected {len(LABEL_COLUMNS)} labels in distribution key {key!r}"
            )
        result[parts] = int(count)  # type: ignore[index]
    return result
...
def run_step_5(
    ...
    require_columns(
        input_path,
        reader.fieldnames,
        ("Projectname", "Filepath", "Function", *LABEL_COLUMNS),
    )
    try:
...
def label_stratum(value: str) -> Stratum:
    ...
    if not isinstance(label, list) or len(label) != len(LABEL_COLUMNS):
        raise ValueError(
            f"Label must be [OSPR, DEVIGN, BIGVUL, MAT], got {value!r}"
        )
    return tuple(as_bool(item) for item in label)  # type: ignore[return-value]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if should_run(5, paths[5], args.force_from):
        ...
        if not original or not available:
            raise RuntimeError(
                "Steps 3 and 4 need four-label distribution metadata. "
                "Rebuild with --force-from 3."
            )
